...
paella.addPlugin(function() {
	return class CaptionsEditorPlugin extends paella.ButtonPlugin {
		...
		showUI() {
			if (self._open || (paella.captions.getAvailableLangs().length >= 1)) {
				super.showUI();
			}
			else {
				super.hideUI();
		...
		setup() {
			...
			// HIDE UI IF NO Captions || !paella.captions.getActiveCaptions()
		...
		buildBodyContent() {
			...
			self._activeCaptions && self._activeCaptions._captions.forEach(function(segment, idx) {
				...
				segmentButton.appendChild(segmentButtonIcon)
				segmentDiv.appendChild(segmentButton);				
